...
;backup old previous mbr into buf1, put new mbr into previous_mbr
	mov si, ADDR_SBMK_PREVIOUS_MBR
	mov cx, SECTOR_SIZE
	call main_xchg_buffer

;backup sbmk header data into buf2
        xor si, si
...
;set some variate
        xor eax, eax
	inc al
	mov cl, [ADDR_SBMK_SECTORS]

	mov [ADDR_SBMK_DRVID], dl
	mov [ADDR_SBMK_BLOCK_MAP], cl
	mov [ADDR_SBMK_BLOCK_MAP+1], eax    ; kernel address = 1
	mov [ADDR_SBMK_BLOCK_MAP+SIZE_OF_STRUC_BLOCK_MAP], ah
                                              ; only one block of kernel
	mov [ADDR_SBMK_CHECKSUM], ah        ; set checksum to zero

        mov [ADDR_SBMK_SBML_CODES + struc_sbml_header.block_map], cl
        mov [ADDR_SBMK_SBML_CODES + struc_sbml_header.block_map+1], eax
	mov [ADDR_SBMK_SBML_CODES + struc_sbml_header.block_map+SIZE_OF_STRUC_BLOCK_MAP], ah
...
;copy sbml to disk_buf1
	push di
	mov si, ADDR_SBMK_SBML_CODES
	mov cx, SIZE_OF_MBR
	rep movsb
	pop di
